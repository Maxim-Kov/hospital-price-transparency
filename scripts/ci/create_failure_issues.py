...
"""
    ...
    if category == "url-update":
        body += """**Type:** URL changed (HTTP 404)
...
"""
        ...
        body += """**Type:** Access blocked (HTTP 403)

Possible causes: IP blocking, User-Agent restrictions, rate limiting

**Manual investigation required.**
"""
    elif category == "registry-update":
        body += """**Type:** No scraper registered for URL pattern

To trigger auto-fix: Add the `registry-update` label

*Uses Claude API credits*
"""
    elif category == "parser-fix":
        body += """**Type:** Parser error (format changed)

The file format may have changed (new columns, different structure).

To trigger auto-fix: Add the `parser-fix` label

*Uses Claude API credits*
"""
    elif category == "encoding-fix":
        body += """**Type:** Character encoding issue

To trigger auto-fix: Add the `encoding-fix` label

*Uses Claude API credits*
"""
    elif category == "scraper-fix":
        body += """**Type:** Scraper logic error

The scraper needs adjustment for this domain.

To trigger auto-fix: Add the `scraper-fix` label

*Uses Claude API credits*
"""
    else:
        body += """**Type:** Unknown - manual investigation required
"""
...
def get_labels_for_category(category: str) -> list[str]:
    ...
    Note: We intentionally do NOT auto-add trigger labels like 'url-update'
    or 'parser-fix' to avoid automatic Claude API calls.
    A human must manually add these labels to opt-in to AI-assisted fixes.
    """
    base_labels = ["scrape-failure"]

    # Descriptive labels only - these do NOT trigger workflows
    category_labels = {
        "url-update": ["type:url-change"],
        "url-blocked": ["type:blocked"],
        "registry-update": ["type:no-scraper"],
        "parser-fix": ["type:parser-error"],
        "encoding-fix": ["type:encoding"],
        "scraper-fix": ["type:scraper-error"],
